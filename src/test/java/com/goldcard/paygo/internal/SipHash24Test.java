...
public class SipHash24Test {
    ...
    @Test
    public void matchesOfficialVectors() {
        byte[] key = new byte[16];
        byte[] input = new byte[64];
        for (int i = 0; i < 16; i++) key[i] = (byte) i;
        for (int i = 0; i < input.length; i++) input[i] = (byte) i;

        for (int length = 0; length < VECTORS.length; length++) {
            byte[] message = new byte[length];
            System.arraycopy(input, 0, message, 0, length);
            assertEquals("vector " + length,
                    VECTORS[length], String.format("%016x", SipHash24.hash(key, message)));
        }
    }

    @Test(expected = IllegalArgumentException.class)
    public void rejectsWrongKeyLength() {
        SipHash24.hash(new byte[15], new byte[0]);
    }
}
